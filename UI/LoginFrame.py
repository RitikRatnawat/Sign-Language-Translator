...
def login(user, code,root):
    ...
    # elif user.get() == "test" and code.get() == "1234":
    #     messagebox.showinfo("Login Successful", "Welcome to Sign Language Translator")
    
    else: 
        conn = mysql.connector.connect(host="localhost", user="root", password="0000", database='user_details')
        my_cursor = conn.cursor()
        my_cursor.execute("select * from records where username=%s and password=%s", (user.get(), code.get()))
